...
import { extractPagesFromBuffer } from "@/lib/pdf-ops";
...
export async function POST(req: Request) {
  try {
    ...
    if (!Array.isArray(body.indices) || body.indices.length === 0) {
      return jsonError("Provide a non-empty indices array (0-based pages to keep).");
    }
    ...
    const out = await extractPagesFromBuffer(buf, indices);
    return pdfResponse(out, "extracted.pdf");
  } catch (e) {

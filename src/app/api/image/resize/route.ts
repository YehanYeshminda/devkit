...
import type { ResizeFit } from "@/lib/image-ops";
import { sharpResize } from "@/lib/image-ops";
...
export async function POST(req: Request) {
  try {
    ...
    if (!["contain", "cover", "fill"].includes(opts.fit)) {
      return jsonError("fit must be contain, cover, or fill.", 400);
    }
    const out = await sharpResize(input, {
      width: Math.floor(Number(opts.width)),
      height: Math.floor(Number(opts.height)),
      fit: opts.fit,
      withoutEnlargement: Boolean(opts.withoutEnlargement),
      format: opts.format,
      quality: opts.quality,
    });
    const stem = safeStem(opts.filename, "image");
    return imageResponse(out.buffer, out.contentType, `${stem}-resized.${out.ext}`);
  } catch (e) {
    ...
    if (msg.startsWith("INVALID_") || msg === "BAD_IMAGE") {
      return jsonError("Invalid image or resize parameters.", 400);
    }

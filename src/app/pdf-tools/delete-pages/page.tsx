...
function PageGrid({ count, selected, onChange, danger = false }: { count: number; selected: Set<number>; onChange: (s: Set<number>) => void; danger?: boolean }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">{selected.size} page{selected.size !== 1 ? "s" : ""} selected{selected.size > 0 ? ` · ${count - selected.size} will remain` : ""}</span>
      </div>
      <div className="max-h-52 overflow-y-auto rounded-lg border border-white/10 bg-white/[0.02] p-2">
        <div className="grid grid-cols-8 gap-1.5 sm:grid-cols-10 lg:grid-cols-12">
          {Array.from({ length: count }, (_, i) => (
            <button key={i} onClick={() => toggle(i)} className={cn("rounded border py-1.5 text-xs font-mono transition-colors", selected.has(i) ? (danger ? "border-red-500/40 bg-red-500/20 text-red-400" : "border-[#6366f1]/40 bg-[#6366f1]/20 text-[#6366f1]") : "border-white/10 bg-white/[0.04] text-muted-foreground hover:bg-white/[0.08]")}>
              {i + 1}
            </button>
          ))}
        </div>
...
export default function DeletePagesPage() {
  ...
  const canProcess = !!sourceBuf && selected.size > 0 && remaining > 0;
        ...
        <div className="space-y-5">
          ...
          {pageCount > 0 && (
            <div className="rounded-xl border border-white/10 bg-card/60 p-5 space-y-4">
              <div>
                <p className="mb-1 text-sm font-medium">Select pages to <span className="text-red-400">delete</span></p>
                <p className="text-xs text-muted-foreground">Click page numbers to mark them for deletion. Red = will be deleted.</p>
              </div>
              <PageGrid count={pageCount} selected={selected} onChange={setSelected} danger />
              {selected.size > 0 && remaining > 0 && (
                <p className="text-xs text-muted-foreground">Result: {remaining} page{remaining !== 1 ? "s" : ""} will remain in the output PDF.</p>
              )}
            </div>
          )}

          {error && <p className="rounded-lg border border-red-500/20 bg-red-500/10 px-4 py-2.5 text-sm text-red-400">{error}</p>}
          {done && <div className="flex items-center gap-2 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-4 py-2.5 text-sm text-emerald-400"><Check className="size-4" /> PDF downloaded with {selected.size} page{selected.size !== 1 ? "s" : ""} removed.</div>}

          <button onClick={deletePages} disabled={processing || !canProcess} className={cn("flex w-full items-center justify-center gap-2 rounded-lg py-3 text-sm font-medium transition-colors", canProcess && !processing ? "bg-[#6366f1] text-white shadow-lg shadow-[#6366f1]/20 hover:bg-[#4f51d4]" : "bg-white/[0.06] text-muted-foreground")}>
            {processing ? <><Loader2 className="size-4 animate-spin" /> Processing…</> : <><Download className="size-4" />Delete {selected.size > 0 ? `${selected.size} Page${selected.size !== 1 ? "s" : ""} &` : ""} Download</>}
          </button>
        </div>
      </main>
    </div>
